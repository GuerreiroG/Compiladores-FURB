...
import static br.edu.furb.compilador.gals.ParserConstants.*;
...
public class Sintatico implements Constants {
    ...
    private void lancarErroDeConstante(int id, int x) throws SyntaticError {
        switch (id) {
            case 4 -> {
                throw new SyntaticError(PARSER_ERROR[x], currentToken.getPosition(),"constante_int");
            }
            case 5 -> {
                throw new SyntaticError(PARSER_ERROR[x], currentToken.getPosition(),"constante_float");
            }
            case 6 -> {
                throw new SyntaticError(PARSER_ERROR[x], currentToken.getPosition(), "constante_string");
            }
        }
    }
